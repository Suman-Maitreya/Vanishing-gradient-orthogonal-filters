...
# Read Accuracy
with open(os.path.join(results_dir, 'vgg_accuracy.csv'), 'r') as f:
    reader = csv.reader(f)
...
# Read Loss
with open(os.path.join(results_dir, 'vgg_loss.csv'), 'r') as f:
    reader = csv.reader(f)
    next(reader) # skip header
    for row in reader:
...
plt.title('VGG-16 + OAF Head: Training Progress', fontsize=14, fontweight='bold')
fig.tight_layout()

# Save the plot
save_path = os.path.join(results_dir, 'vgg_training_metrics.png')
